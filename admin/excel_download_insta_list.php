...
if($startDate != "" && $endDate != "") {
    $where = "AND verify_regdate >= '".$startDate."' AND verify_regdate <= '".$endDate." 23:59:59'";
}
$list_query = "SELECT * FROM verify_info_9 WHERE 1 ".$where."";
...
{
    $upload_image = "https://www.hi-maumbot.co.kr/uploads/".$val["verify_directory"]."/".$val["verify_file_name"];
    
    
    $objPHPExcel->setActiveSheetIndex(0)
                ->setCellValue('A'.$dataIdx, $val['verify_name'])
                ->setCellValue('B'.$dataIdx, $val['verify_phone'])
                ->setCellValue('C'.$dataIdx, $upload_image)
                ->setCellValue('D'.$dataIdx, $val['verify_desc'])
                ->setCellValue('E'.$dataIdx, $val['verify_tag'])
                ->setCellValue('F'.$dataIdx, $val['verify_media'])
                ->setCellValue('G'.$dataIdx, $val['verify_gubun'])
                ->setCellValue('H'.$dataIdx, $val['verify_regdate']);
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('현대해상_건강키트_인증이벤트_참여자목록_'.date("Ymd"));
...
header('Content-Disposition: attachment;filename="현대해상_건강키트_인증이벤트_참여자데이터_'.date("Ymd").'.xls"');
