          ...
          <div class="table-responsive">
            ...
            <div id="daily_applicant_count_div1" style="display:block">
              <table class="table table-hover">
                ...
                <tbody>
<?php
	$daily_date_query	= "SELECT mb_regdate FROM member_info_9 WHERE 1 Group by substr(mb_regdate,1,10) ORDER BY mb_regdate DESC";
	$date_res			= mysqli_query($my_db, $daily_date_query);
	while($date_daily_data = mysqli_fetch_array($date_res))
	{
		$daily_date		= substr($date_daily_data['mb_regdate'],0,10);
		$media_query	= "SELECT mb_media, COUNT( mb_media ) media_cnt FROM member_info_9 WHERE 1 AND mb_regdate LIKE  '%".$daily_date."%' GROUP BY mb_media";
		$media_res		= mysqli_query($my_db, $media_query);

		$unique_query	= "SELECT * FROM member_info_9 WHERE 1 AND mb_regdate LIKE  '%".$daily_date."%' GROUP BY mb_ipaddr";
		...
		while ($media_daily_data = mysqli_fetch_array($media_res))
		{
			...
			$pc_query		= "SELECT * FROM member_info_9 WHERE 1 AND mb_regdate LIKE  '%".$daily_date."%' AND mb_media='".$media_daily_data['mb_media']."' AND mb_gubun='PC'";
			$pc_count		= mysqli_num_rows(mysqli_query($my_db, $pc_query));
			$mobile_query	= "SELECT * FROM member_info_9 WHERE 1 AND mb_regdate LIKE  '%".$daily_date."%' AND mb_media='".$media_daily_data['mb_media']."' AND mb_gubun='MOBILE'";
