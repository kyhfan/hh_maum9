...
//	$query = "SELECT * FROM winner_list_lms WHERE 1 AND winner_phone IN ('01037578414','01029280422') AND send_YN = 'N'";
	$query = "SELECT * FROM winner_list_lms WHERE 1 AND send_YN = 'N'";
...
//	$query = "SELECT * FROM winner_list_lms WHERE 1 AND winner_phone LIKE '01029280422'";
	...
	if($result) {
		while ($data = mysqli_fetch_array($result))
		{
			...
			switch($data["winner_type"]) {
				case '2nd':
					$response = sendRequest_winner_2nd($httpmethod, $url, $clientKey, $contentType, $data);	
				break;
				case '3rd':
...
//			발송여부 업데이트
			if($json_data['result_code'] == '200') {
				$updateQry = "UPDATE winner_list_lms SET send_YN = 'Y', send_date = '".date("Y-m-d H:i:s")."' WHERE 1 AND winner_phone = '".$data["winner_phone"]."'";
